...
import NPCTradeTab from './tabs/NPCTradeTab'; // Import Trade Tab
import NPCQuestsTab from './tabs/NPCQuestsTab'; // Import Quests Tab
  ...
  const availableTabs = useMemo(() => {
    const allTabs = [
      ...
      },
      {
        id: 'trade',
        label: 'Trade',
        icon: <ChatIcon />, // Placeholder icon, replace with appropriate icon
        disabled: npc.relationshipValue < 2, // Example: requires relationship level 2+
        tooltip: npc.relationshipValue < 2 ? 'Requires relationship level 2+' : 'Trade items with NPC'
      },
      {
        id: 'quests',
        label: 'Quests',
        icon: <AssignmentIcon />, // Placeholder icon, replace with appropriate icon
        disabled: npc.relationshipValue < 3, // Example: requires relationship level 3+
        tooltip: npc.relationshipValue < 3 ? 'Requires relationship level 3+' : 'Manage quests with NPC'
      }
    ];
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}>
        ...
        {/* Display Connection Depth */}
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Typography variant="body2" color="text.secondary">
            Connection Depth:
          </Typography>
          <Chip
            label={npc.connectionDepth.toFixed(1)}
            size="small"
            color={npc.connectionDepth >= 50 ? 'info' : 'default'} // Example coloring
          />
        </Box>
      </Box>
      ...
      >
        ...
        {activeTab === 'trade' && npc.relationshipValue >= 2 && (
          <NPCTradeTab npc={npc} relationshipLevel={npc.relationshipValue} />
        )}
        {/* Quests Tab - Relationship 3+ */}
        {activeTab === 'quests' && npc.relationshipValue >= 3 && (
          <NPCQuestsTab npc={npc} relationshipLevel={npc.relationshipValue} />
        )}
      </TabContainer>
    </Box>
  );
