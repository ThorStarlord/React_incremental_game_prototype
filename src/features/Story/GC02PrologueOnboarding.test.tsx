...
import { CITY_CENTER_LOCATION_ID, WHISPERING_WOODS_LOCATION_ID } from '../Exploration/LocationDefinitions';
import {
...
describe('GC-02 Prologue / onboarding vertical path', () => {
  ...
  test('projects one clear ordinary-UI objective from fresh play through Willow relationship evidence to Chapter 1', () => {
    ...
    expect(screen.getByRole('link', { name: 'Open travel controls' })).toHaveAttribute('href', '/game/dashboard');
  ...
  test('persistent first-lesson evidence survives save/load and preserves the Chapter 1 handoff', async () => {
    ...
    expect(resumedStore.getState().relationships.experiencesById[GC02_FIRST_LESSON_EXPERIENCE_ID]).toBeDefined();
