...
/** Compute essence cost to create a Copy from an NPC, scaled by connectionDepth. */
export const getCopyCreationCost = (connectionDepth: number | undefined): number => {
  const depth = Math.max(0, connectionDepth ?? 0);
  const discount = depth * COPY_SYSTEM.CREATION_DISCOUNT_PER_DEPTH;
  const raw = COPY_SYSTEM.CREATION_BASE_COST - discount;
  return Math.max(COPY_SYSTEM.CREATION_MIN_COST, Math.floor(raw));
};
