...
describe('GameLoop large-frame/background-stall preflight', () => {
  ...
  test('characterizes an unpaused 5s RAF gap as 50 ordinary live fixed steps at 10 Hz', () => {
  ...
  test('one 5s catch-up frame and fifty regular 100ms frames converge to the same scheduler state', () => {
  ...
  test('preserves a large-frame fractional remainder instead of dropping it after catch-up', () => {
    const store = makeStore();
  ...
  test('a 5s catch-up burst advances scheduler state immediately while async onTick work stays serialized', async () => {
    const store = makeStore();
