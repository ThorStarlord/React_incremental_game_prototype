 ...
 *
 * The policy was selected before the production repair. In this composed
 * candidate the same model remains the executable semantic oracle while the
 * real useGameLoop implementation is expected to satisfy the one-tick bound.
 */
...
describe('GameLoop async tick backlog policy contract', () => {
  ...
  test('repaired production satisfies the selected one-tick producer-lead bound while an async consumer is unresolved', () => {
    ...
    try {
      ...
      expect(producerLead).toBe(0);
      expect(producerLead).toBeLessThanOrEqual(1);
    } finally {
      mounted.unmount();
      releaseFirst?.();
      raf.restore();
