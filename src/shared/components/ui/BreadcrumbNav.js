 ...
 * @param {boolean} [props.testMode=false] - Enable test mode with sample paths
 * @returns {JSX.Element} The styled breadcrumb navigation component
 */
const BreadcrumbNav = ({ testMode = false }) => {
  // Always call hooks at the top level of your component
  const location = useLocation();
  const navigate = useNavigate();
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down('sm'));
  const [testPathIndex, setTestPathIndex] = useState(0);
   ...
   * @returns {Object} - Style object with hover settings
   */
  const getBreadcrumbItemStyle = (id) => ({
    display: 'flex',
    alignItems: 'center',
   ...
   */
  const shouldShowBreadcrumb = (index) => {
