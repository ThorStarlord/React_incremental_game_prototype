...
*   **Feature-Sliced Structure:** Continue using the feature-sliced approach (`src/features/FeatureName/`). This promotes modularity and scalability.
    *   **Convention:** Feature directories should be **singular and `PascalCase`** (e.g., `src/features/Player`, `src/features/Trait`, `src/features/Copy`, `src/features/NPC`). The corresponding Redux slice key should be **singular and `camelCase`** (e.g., `player`, `trait`, `copy`, `npc`).
